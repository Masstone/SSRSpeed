	...
	#	self._config["server_port"] = int(self._config["server_port"])
		...
		if (self._process == None):
			if (self._checkPlatform() == "Windows"):
				...
				if (logger.level == logging.DEBUG):
					self._process = subprocess.Popen(["python","./clients/shadowsocksr/shadowsocks/local.py","-v","-c","%s/config.json" % os.getcwd()])
				else:
					self._process = subprocess.Popen(["python","./clients/shadowsocksr/shadowsocks/local.py","-c","%s/config.json" % os.getcwd()],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
